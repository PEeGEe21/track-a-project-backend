...
export class ProjectUpdatesController {
  ...
  @Get('reference-options') referenceOptions(@Req() req: any, @Headers('x-organization-id') org: string, @Param('projectId', ParseIntPipe) projectId: number, @Query('type') type?: 'task' | 'milestone' | 'document' | 'user', @Query('search') search?: string, @Query('page') page?: number, @Query('limit') limit?: number) { return this.service.referenceOptions(req.user, org, projectId, type, search, page, limit); }
