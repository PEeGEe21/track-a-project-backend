...
import {
  ...
  ProjectUpdateReferenceType,
} from 'src/typeorm/entities/ProjectUpdateReference';
...
export class ProjectUpdatesService {
  constructor(
    ...
    private readonly updates: Repository<ProjectUpdate>,
  ...
  async list(
    ...
    let statuses =
      includeDrafts && access.canCreate
        ? [ProjectUpdateStatus.DRAFT, ProjectUpdateStatus.PUBLISHED]
        : [ProjectUpdateStatus.PUBLISHED];
    if (filters.status === ProjectUpdateStatus.DRAFT && access.canCreate)
      statuses = [ProjectUpdateStatus.DRAFT];
    if (filters.status === ProjectUpdateStatus.PUBLISHED)
      statuses = [ProjectUpdateStatus.PUBLISHED];
    ...
    if (
      Object.values(ProjectUpdateHealth).includes(
        filters.health as ProjectUpdateHealth,
      )
    )
      base.health = filters.health;
    ...
    if (
      ...
      access.canCreate &&
      !access.canManage &&
      filters.status !== ProjectUpdateStatus.PUBLISHED
    ) {
      where = [
        { ...base, status: ProjectUpdateStatus.PUBLISHED },
        {
          ...base,
          status: ProjectUpdateStatus.DRAFT,
          author_id: access.user.id,
        },
      ];
    }
    const [data, total] = await this.updates.findAndCount({
      where,
      relations: ['author', 'references'],
    ...
    return {
      data,
      meta: this.meta(pagination.page, pagination.limit, total, data.length),
    };
  }
  ...
  async get(
    ...
    if (
      update.status === ProjectUpdateStatus.DRAFT &&
      !access.canManage &&
      Number(update.author_id) !== Number(access.user.id)
    )
  ...
  async referenceOptions(
    ...
    organizationId: string,
    projectId: number,
    type: 'task' | 'milestone' | 'document' | 'user' = 'task',
    search = '',
    page = 1,
    limit = 20,
  ) {
    ...
    if (type === 'task') {
      ...
      return {
        ...
        meta: this.meta(pagination.page, pagination.limit, total, items.length),
      };
    }
    if (type === 'document') {
  ...
  async createDraft(
    ...
    organizationId: string,
    projectId: number,
    dto: SaveProjectUpdateDto,
  ) {
    const access = await this.assertAccess(actor, organizationId, projectId);
    if (!access.canCreate)
      throw new ForbiddenException(
        'Contributor access is required to create project update drafts',
      );
    return this.dataSource.transaction(async (manager) => {
      const update = await manager.save(
        ProjectUpdate,
        manager.create(ProjectUpdate, {
          ...this.content(dto),
          project_id: projectId,
          organization_id: organizationId,
          author_id: access.user.id,
      ...
      update.references = await this.buildReferences(
        dto,
        organizationId,
        projectId,
        manager.getRepository(ProjectUpdateReference),
        update.id,
      );
      await manager.save(update.references);
      return manager.findOne(ProjectUpdate, {
        where: { id: update.id },
        relations: ['author', 'references'],
      });
    });
  ...
  async updateDraft(
    ...
    const access = await this.assertAccess(actor, organizationId, projectId);
    const update = await this.findUpdate(organizationId, projectId, updateId);
    if (update.status !== ProjectUpdateStatus.DRAFT)
      throw new BadRequestException(
        'Published updates are immutable; create a correction instead',
      );
    if (
      !access.canManage &&
      Number(update.author_id) !== Number(access.user.id)
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      await manager.save(update);
      await manager.delete(ProjectUpdateReference, {
        project_update_id: update.id,
      });
      const refs = await this.buildReferences(
        dto,
        organizationId,
        projectId,
        manager.getRepository(ProjectUpdateReference),
  ...
  async deleteDraft(
    ...
    if (update.status !== ProjectUpdateStatus.DRAFT)
      throw new BadRequestException(
        'Published updates cannot be deleted; publish a correction instead',
      );
    if (
      !access.canManage &&
      Number(update.author_id) !== Number(access.user.id)
  ...
  async publish(
    ...
    if (!access.canManage)
      throw new ForbiddenException(
        'Editor access is required to publish project updates',
      );
    const update = await this.findUpdate(organizationId, projectId, updateId);
    if (update.status !== ProjectUpdateStatus.DRAFT)
      throw new BadRequestException('Update is already published');
    update.status = ProjectUpdateStatus.PUBLISHED;
  ...
  async correct(
    ...
    if (!access.canCreate)
      throw new ForbiddenException(
        'Contributor access is required to correct project updates',
      );
    const original = await this.findUpdate(organizationId, projectId, updateId);
    if (original.status !== ProjectUpdateStatus.PUBLISHED)
      throw new BadRequestException('Only published updates can be corrected');
    if (
      Number(original.author_id) !== Number(access.user.id) &&
      !access.isOwner &&
      actor.role !== 'super_admin'
    )
      throw new ForbiddenException(
        'Only the author or project owner can correct this update',
      );
    const correction = await this.dataSource.transaction(async (manager) => {
      const seriesId = original.series_id ?? original.id;
      const latest = await manager.findOne(ProjectUpdate, {
      ...
      const next = await manager.save(
        ...
        manager.create(ProjectUpdate, {
          ...
          series_id: seriesId,
          corrects_update_id: original.id,
          version: (latest?.version ?? original.version) + 1,
        }),
      );
      next.references = await this.buildReferences(
        dto,
        organizationId,
        projectId,
        manager.getRepository(ProjectUpdateReference),
  ...
  async history(
    ...
    const [data, total] = await this.updates.findAndCount({
      where: {
        series_id: update.series_id ?? update.id,
        status: ProjectUpdateStatus.PUBLISHED,
      },
      relations: ['author', 'references'],
      order: { version: 'DESC' },
  ...
  private content(dto: SaveProjectUpdateDto) {
    if (
      dto.reporting_period_start &&
      dto.reporting_period_end &&
      dto.reporting_period_start > dto.reporting_period_end
    )
      throw new BadRequestException(
        'Reporting period start must not be after its end',
      );
    return {
      health: dto.health,
      accomplishments: dto.accomplishments?.trim() || null,
      blockers: dto.blockers?.trim() || null,
      next_steps: dto.next_steps?.trim() || null,
      reporting_period_start: dto.reporting_period_start ?? null,
      reporting_period_end: dto.reporting_period_end ?? null,
    };
  }

  private async findUpdate(org: string, projectId: number, id: number) {
    const update = await this.updates.findOne({
  ...
  private async assertAccess(actor: any, org: string, projectId: number) {
    ...
    const isCreator = Number(project.user?.id) === Number(user.id);
    if (!isCreator && !peer && actor.role !== 'super_admin')
      throw new ForbiddenException('You do not have access to this project');
    const role =
      isCreator || actor.role === 'super_admin'
        ? ProjectRole.OWNER
        : peer?.role ?? ProjectRole.EDITOR;
  ...
  private async buildReferences(
    ...
    org: string,
    projectId: number,
    repo: Repository<ProjectUpdateReference>,
    updateId: number,
  ) {
    const refs: ProjectUpdateReference[] = [];
    for (const ref of dto.references ?? []) {
      let label = ref.label;
      if (ref.type === ProjectUpdateReferenceType.TASK) {
        const item = await this.tasks.findOne({
          where: {
            id: Number(ref.id),
            organization_id: org,
            project: { id: projectId },
        ...
        if (!item)
          throw new BadRequestException(`Invalid task reference ${ref.id}`);
        label = item.title;
      }
      if (ref.type === ProjectUpdateReferenceType.DOCUMENT) {
        ...
        if (!item)
          throw new BadRequestException(`Invalid document reference ${ref.id}`);
        label = item.title;
      }
      if (ref.type === ProjectUpdateReferenceType.USER) {
        const member = await this.memberships.findOne({
          where: { organization_id: org, user: { id: Number(ref.id) } },
        });
        if (!member)
          throw new BadRequestException(`Invalid user reference ${ref.id}`);
        const item = await this.users.findOne({
          where: { id: Number(ref.id) },
        });
        label = item?.fullName || item?.email;
      }
      if (ref.type === ProjectUpdateReferenceType.MILESTONE) {
        const item = await this.milestones.findOne({
          where: {
            id: ref.id,
            organization_id: org,
            project_id: projectId,
        ...
        if (!item)
          throw new BadRequestException(
            `Invalid milestone reference ${ref.id}`,
          );
        label = item.title;
      }
      refs.push(
  ...
  private async notifyMembers(
    ...
    recipients.set(Number(project.user.id), project.user);
    peers.forEach((p) => recipients.set(Number(p.user.id), p.user));
    recipients.delete(Number(sender.id));
    await Promise.all(
      [...recipients.values()].map((recipient) =>
        this.notifications.createNotification(
          sender,
          {
            recipient,
            sender,
            title: `New update: ${project.title}`,
            message: 'A project update was published.',
            type: 'project_update',
            metadata: {
              projectId: project.id,
              projectUpdateId: update.id,
              deliveryKey: `project-update:${update.id}:${recipient.id}`,
            },
          },
          org,
        ),
